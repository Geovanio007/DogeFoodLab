...
class DogeLabAPITester:
    ...
    # ========== RARITY SYSTEM TESTING (RENDER BACKEND) ==========
    ...
    def test_rarity_system_treat_creation_2_ingredients(self):
        ...
        if success and response:
            ...
            # Verify rarity is Common or Uncommon only
            if rarity not in ['Common', 'Uncommon']:
                print(f"   ❌ Invalid rarity for 2 ingredients: {rarity} (should be Common or Uncommon)")
                return False, response
    ...
    def test_rarity_system_treat_creation_4_ingredients(self):
        ...
        if success and response:
            ...
            # Verify rarity is up to Epic (no Legendary/Mythic)
            allowed_rarities = ['Common', 'Uncommon', 'Rare', 'Epic']
            if rarity not in allowed_rarities:
                print(f"   ❌ Invalid rarity for 4 ingredients: {rarity} (should be up to Epic)")
                return False, response
    ...
    def test_rarity_system_comprehensive(self):
        """Comprehensive test of the rarity system"""
        print("\n🚀 COMPREHENSIVE RARITY SYSTEM TEST")
        print("=" * 60)
    ...
    def test_final_production_readiness(self):
        ...
        print(f"   URL: https://doge-build-test.preview.emergentagent.com")
    ...
    def test_render_backend_migration(self):
        ...
        print("\n🚀 RENDER BACKEND MIGRATION TEST")
        print("=" * 70)
        print("Testing DogeFood Lab Game on new Render infrastructure")
        print("Frontend: https://app-bcktxract-dogefoods-projects.vercel.app")
        print("Backend: https://dogefood-lab-api.onrender.com")
        ...
        # 1. Health Check - Critical endpoint
        ...
        if success and response:
            ...
            if status == 'healthy' and database == 'connected':
                ...
                print("   ❌ Health check FAILED - Backend issues detected")
                all_tests_passed = False
        else:
            test_results['health_check'] = False
            print("   ❌ Health check endpoint failed")
        ...
        if success and response:
            leaderboard_count = len(response) if isinstance(response, list) else 0
            print(f"   ✅ Leaderboard retrieved: {leaderboard_count} players")
            
            if leaderboard_count > 0:
                # Check first entry structure
                first_entry = response[0]
                required_fields = ['address', 'points', 'level', 'rank']
                missing_fields = [field for field in required_fields if field not in first_entry]
                
                if not missing_fields:
                    print(f"   ✅ Leaderboard structure correct")
                    print(f"   📊 Top player: {first_entry.get('points', 0)} points, Level {first_entry.get('level', 0)}")
                    test_results['leaderboard'] = True
                else:
                    print(f"   ❌ Leaderboard missing fields: {missing_fields}")
                    test_results['leaderboard'] = False
                    all_tests_passed = False
            else:
                print(f"   ⚠️ Empty leaderboard - may be expected for new deployment")
                test_results['leaderboard'] = True  # Empty is acceptable
        else:
            test_results['leaderboard'] = False
        ...
        if success and response:
            guest_address = response.get('address')
            guest_level = response.get('level', 0)
            guest_points = response.get('points', 0)
            
            print(f"   ✅ Guest player created successfully")
            print(f"   📱 Address: {guest_address}")
            print(f"   📊 Level: {guest_level}")
            print(f"   ⭐ Points: {guest_points}")
            
            if guest_address == self.guest_address:
                test_results['guest_registration'] = True
                print("   🎯 Guest registration PASSED")
            else:
                test_results['guest_registration'] = False
                print("   ❌ Guest registration data mismatch")
                all_tests_passed = False
        else:
            test_results['guest_registration'] = False
            print("   ❌ Guest player registration failed")
            all_tests_passed = False
        ...
        if success and response:
            ...
            print(f"   ✅ Ingredients retrieved: {ingredient_count} available")
            ...
            if ingredient_count > 0:
                ...
                if not missing_fields:
                    print(f"   ✅ Ingredient structure correct")
                    print(f"   🥕 Sample ingredient: {first_ingredient.get('name', 'Unknown')}")
                    test_results['ingredients'] = True
                else:
                    print(f"   ❌ Ingredient missing fields: {missing_fields}")
                    test_results['ingredients'] = False
                    all_tests_passed = False
            else:
            ...
            test_results['ingredients'] = False
            print("   ❌ Ingredients endpoint failed")
            all_tests_passed = False
        ...
        treat_data = {
            "creator_address": self.guest_address,
            "ingredients": ["chicken", "bones"],
            "player_level": 1
        }
        ...
        if success and response:
            ...
            print(f"   ✅ Treat created successfully")
            print(f"   🍖 Treat ID: {treat_id}")
            print(f"   🎯 Rarity: {rarity}")
            print(f"   ⏰ Timer: {timer_hours} hours")
            
            if treat_id and rarity:
                test_results['treat_creation'] = True
                print("   🎯 Treat creation PASSED")
                
                # Store treat ID for timer testing
                self.test_treat_id = treat_id
            else:
                test_results['treat_creation'] = False
                print("   ❌ Treat creation incomplete")
                all_tests_passed = False
        else:
            test_results['treat_creation'] = False
            print("   ❌ Treat creation failed")
            all_tests_passed = False
        
        # 6. Active Treats Timer
        ...
        success, response = self.run_test(
            "Active Treats Timer", 
            "GET", 
            f"treats/{self.guest_address}/active", 
            200
        )
        
        if success and response:
            treats = response.get('treats', [])
            server_time = response.get('server_time', 'Unknown')
            treat_count = response.get('count', 0)
            
            print(f"   ✅ Active treats retrieved: {treat_count} treats")
            print(f"   🕐 Server time: {server_time}")
            
            if treats:
                # Check timer data structure
                first_treat = treats[0]
                timer_data = first_treat.get('timer', {})
                
                if timer_data:
                    remaining_seconds = timer_data.get('remaining_seconds', 0)
                    remaining_formatted = timer_data.get('remaining_formatted', 'Unknown')
                    is_ready = timer_data.get('is_ready', False)
                    progress_percent = timer_data.get('progress_percent', 0)
                    
                    print(f"   ✅ Timer data structure correct")
                    print(f"   ⏰ Time remaining: {remaining_formatted}")
                    print(f"   📊 Progress: {progress_percent}%")
                    print(f"   🎯 Ready: {is_ready}")
                    
                    test_results['active_treats_timer'] = True
                    print("   🎯 Active treats timer PASSED")
                else:
                    print(f"   ❌ Timer data missing")
                    test_results['active_treats_timer'] = False
                    all_tests_passed = False
            else:
                print(f"   ⚠️ No active treats found - may be expected")
                test_results['active_treats_timer'] = True  # No treats is acceptable
        else:
            test_results['active_treats_timer'] = False
            print("   ❌ Active treats timer endpoint failed")
            all_tests_passed = False
        ...
        if success and response:
            ...
            if 'DogeFood Lab' in message:
                test_results['frontend_backend_communication'] = True
                print("   🎯 Frontend-Backend communication PASSED")
            else:
                test_results['frontend_backend_communication'] = False
        ...
        # Final Assessment
        print("\n" + "="*70)
        print("🎯 RENDER BACKEND MIGRATION ASSESSMENT")
        print("="*70)
        ...
        print(f"📊 Overall Score: {passed_tests}/{total_tests} tests passed")
        print(f"🔍 Health Check: {'✅ PASS' if test_results.get('health_check') else '❌ FAIL'}")
        print(f"🏆 Leaderboard: {'✅ PASS' if test_results.get('leaderboard') else '❌ FAIL'}")
        print(f"👤 Guest Registration: {'✅ PASS' if test_results.get('guest_registration') else '❌ FAIL'}")
        print(f"🥕 Available Ingredients: {'✅ PASS' if test_results.get('ingredients') else '❌ FAIL'}")
        print(f"🍖 Treat Creation: {'✅ PASS' if test_results.get('treat_creation') else '❌ FAIL'}")
        print(f"⏰ Active Treats Timer: {'✅ PASS' if test_results.get('active_treats_timer') else '❌ FAIL'}")
        print(f"🌐 Frontend-Backend Comm: {'✅ PASS' if test_results.get('frontend_backend_communication') else '❌ FAIL'}")
        
        if all_tests_passed:
            print("\n🚀 RENDER BACKEND MIGRATION: ✅ SUCCESSFUL!")
            print("✅ All critical API endpoints operational")
            print("✅ Database connectivity confirmed")
            print("✅ Game functionality working")
            print("✅ Ready for production use")
        else:
            print("\n⚠️ RENDER BACKEND MIGRATION: 🔶 ISSUES DETECTED")
            failed_tests = [name for name, passed in test_results.items() if not passed]
            print(f"❌ Failed tests: {', '.join(failed_tests)}")
            print("⚠️ Review and fix issues before full deployment")
...
if __name__ == "__main__":
    ...
    base_url = sys.argv[1] if len(sys.argv) > 1 else "https://dogefood-lab-api.onrender.com"
    
    tester = DogeLabAPITester(base_url)
    
    print(f"🧪 DogeFood Lab API Testing Suite")
    print(f"🌐 Testing against: {base_url}")
    print(f"=" * 60)
    
    # Run comprehensive test suite
    try:
        # Run the Render backend migration test
        all_passed, test_results = tester.test_render_backend_migration()
        ...
        if all_passed:
            print(f"\n🚀 ALL TESTS PASSED - RENDER BACKEND IS READY!")
        else:
