...
async def run_heat_event_scheduler(db):
    """
    Background task: rotates heat events on a real 30-minute wall-clock schedule
    regardless of user traffic. Call once at server startup via asyncio.create_task().
    """
    import asyncio
    import logging
    ...
    while True:
        try:
            arena = await db.arena_sessions.find_one({"status": "active"}, {"_id": 0, "heat_event_started_at": 1, "id": 1})
            if arena:
                ...
                if remaining_sec < 5:
                    ...
                    logger.info(f"🔥 Heat event rotated → {new_event['name']} ({new_event['id']})")
                    # Sleep the full duration before next check
                    await asyncio.sleep(HEAT_EVENT_DURATION_MIN * 60)
                else:
                    # Sleep until next rotation
                    await asyncio.sleep(min(remaining_sec, 60))  # check at most every 60s
            else:
            ...
            logger.warning(f"Heat scheduler error (retrying in 60s): {e}")
            await asyncio.sleep(60)
