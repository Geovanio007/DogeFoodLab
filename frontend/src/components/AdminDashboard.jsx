...
const AdminDashboard = () => {
  ...
  const handleAdminAuth = async () => {
    if (!adminKey || authLoading) return;
    setAuthLoading(true);
    try {
      // SECURITY: key is validated server-side, never stored in frontend code
      ...
      toast({
        title: "Access Denied",
        description: "Could not verify admin key",
        variant: "destructive"
      });
    } finally {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load flagged players (admin-protected)
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            ...
            />
            <Button onClick={handleAdminAuth} disabled={authLoading} className="w-full" data-testid="admin-login-btn">
...
const EnhancedSeasonManagement = ({ seasons, onRefresh, toast, adminKey }) => {
...
const GameEngineManagement = ({ onRefresh, toast, adminKey }) => {
...
const SecurityMonitoring = ({ flaggedPlayers, onRefresh, toast, adminKey }) => {
