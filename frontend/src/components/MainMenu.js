      ...
      {count > 1 && (
        <>
          <button
            ...
            className="absolute z-10 left-2 top-1/2 -translate-y-1/2 shrink-0 min-w-0 min-h-0 p-0 m-0 border-0 appearance-none leading-none rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); goTo(index + 1); }}
            className="absolute z-10 right-2 top-1/2 -translate-y-1/2 shrink-0 min-w-0 min-h-0 p-0 m-0 border-0 appearance-none leading-none rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
            style={{ width: 28, height: 28 }}
            aria-label="Next banner"
          >
            <ChevronRight className="w-4 h-4 text-white" />
          </button>
          <div className="absolute z-10 bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1.5">
            {banners.map((banner, i) => (
              <button
                key={banner.id}
                type="button"
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); goTo(i); }}
                className={`shrink-0 min-w-0 min-h-0 p-0 m-0 border-0 appearance-none leading-none block rounded-full transition-all ${i === index ? 'bg-white' : 'bg-white/40 hover:bg-white/60'}`}
