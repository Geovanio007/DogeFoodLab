...
import {
  ...
  Rocket, Swords, Gift, Target, Award, Gem, Flame, BarChart3, Newspaper
} from 'lucide-react';
...
// ─── Arena Heat Event Card ─────────────────────────────────────
// Mirrors the existing "Happy Hour" featured card exactly (same layout,
// same dynamic-border/icon-box language) so the live Arena event reads as
// part of the same "what's live right now" row instead of a bolted-on widget.
const ArenaHeatCard = ({ heat }) => {
  ...
  if (!event) {
    return (
      <div className="relative overflow-hidden rounded-2xl p-5 sm:p-6 border bg-gradient-to-br from-[#151b28] to-[#0d1117] border-white/[0.06]"
        ...
        <div className="relative flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center bg-gradient-to-br from-slate-600 to-slate-700"
            style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}>
            <Flame className="w-8 h-8 text-slate-300 drop-shadow-md" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-white mb-0.5">Arena Event</h3>
            <p className="text-xs text-slate-400">Loading current event…</p>
          </div>
        </div>
      </div>
    );
  ...
  const startedMs = new Date(heat.started_at).getTime();
  const endMs = startedMs + (heat.duration_min || 30) * 60 * 1000;
  ...
  return (
    <Link to="/arena">
      <div className="relative overflow-hidden rounded-2xl p-5 sm:p-6 border transition-all duration-200 hover:-translate-y-0.5"
        style={{
          background: `linear-gradient(135deg, ${event.color}14 0%, #151b28 60%)`,
          borderColor: `${event.color}44`,
          boxShadow: '0 6px 20px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.04)',
        }}
        data-testid="arena-heat-card">
        <div className="absolute inset-0 bg-gradient-to-br from-white/[0.03] to-transparent rounded-2xl pointer-events-none" />
        <div className="absolute top-0 right-0 w-40 h-40 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" style={{ backgroundColor: `${event.color}22` }} />
        <div className="relative flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
            style={{ background: `linear-gradient(135deg, ${event.color}, ${event.color}bb)`, boxShadow: `0 8px 24px ${event.color}55` }}>
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <span className="px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 text-[10px] font-bold animate-pulse shrink-0">LIVE</span>
            </div>
            <p className="text-xs text-slate-400 truncate">{event.blurb}</p>
            <div className="flex items-center gap-2 mt-2">
              <Clock className="w-3 h-3" style={{ color: event.color }} />
              <span className="text-[10px] font-mono font-semibold" style={{ color: event.color }}>{mm}:{ss} left</span>
            </div>
          </div>
        </div>
      </div>
...
// ─── Main Menu Banner Carousel ─────────────────────────────────
// Add/remove entries here to change what shows in the carousel.
// `gated: true` routes the click through the auth-gate handler
// (same behavior as the other Lab entry points).
const MENU_BANNERS = [
  { id: 'crate', image: '/Cratebanner.png', alt: 'Lab Crates', to: '/lab', gated: true, testId: 'banner-crate' },
  { id: 'lab-surge', image: '/LabSurgeBanner.png', alt: 'Lab Surge', to: '/lab-surge', testId: 'banner-lab-surge' },
  { id: 'lab-feed', image: '/LabFeedBanner.png', alt: 'Lab Feed', to: '/lab-feed', testId: 'banner-lab-feed' },
  { id: 'lab-launcher', image: '/LabLauncherBanner.png', alt: 'Lab Launcher', to: '/lab-launcher', testId: 'banner-lab-launcher' },
];

const BannerCarousel = ({ banners, onAuthRequired, intervalMs = 5000 }) => {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => setIndex((i) => (i + 1) % count), intervalMs);
    return () => clearInterval(iv);
  ...
  return (
    <div
      ...
      data-testid="banner-carousel-menu"
      ...
      >
        {banners.map((banner) => (
          <Link
            key={banner.id}
            to={banner.to}
            onClick={banner.gated ? onAuthRequired : undefined}
      ...
      {count > 1 && (
        <>
          <div
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); goTo(index - 1); } }}
            className="absolute z-10 left-2 top-1/2 -translate-y-1/2 cursor-pointer select-none rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          <div
            ...
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); goTo(index + 1); }}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); goTo(index + 1); } }}
            className="absolute z-10 right-2 top-1/2 -translate-y-1/2 cursor-pointer select-none rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
            style={{ width: 28, height: 28 }}
...
/* ─── Booster Package CTA Bubble ─────────────────────────────────────────────
   Floating left-edge widget for the DOGE-priced booster bundle (10-day
   Auto-Mixer + 10 extra lives + 5,000 points). Positioned just above the
   "Game Guide" (?) button (bottom:90, left:16) so the two read as a stack
   in the same left-edge lane instead of competing corners. A small lab
   flask — bubbling green liquid, rising bubbles, steam off the neck — ties
   it to the game's actual mixing-lab theme instead of a generic gift icon.
   ─────────────────────────────────────────────────────────────────────────── */
const BOOSTER_PACKAGE_UI = {
  costDoge: 50,
  autoMixerDays: 10,
  extraLives: 10,
  points: 5000,
};
const BOOSTER_COOLDOWN_HOURS_UI = 48;

const BoosterFlaskIcon = ({ size = 44, dormant = false }) => {
  const bubbleStyle = (delay) => ({
    transformBox: 'fill-box', transformOrigin: 'center',
    animation: dormant ? 'none' : `boosterBubbleRise 2.1s ease-in infinite`,
    animationDelay: delay,
    opacity: dormant ? 0 : undefined,
  });
  const steamStyle = (delay) => ({
    transformBox: 'fill-box', transformOrigin: 'center',
    animation: dormant ? 'none' : `boosterSteamRise 2.6s ease-out infinite`,
    animationDelay: delay,
    opacity: dormant ? 0 : undefined,
  });

  return (
    <svg viewBox="0 0 64 72" width={size} height={size * (72 / 64)} style={{ overflow: 'visible' }}>
      <defs>
        <linearGradient id={dormant ? 'boosterLiquidGrey' : 'boosterLiquid'} x1="0" y1="1" x2="0" y2="0">
          {dormant ? (
            <>
        ...
        <filter id="boosterSteamBlur" x="-60%" y="-60%" width="220%" height="220%">
          <feGaussianBlur stdDeviation="1.5" />
        </filter>
        <clipPath id="boosterFlaskClip">
          <path d="M26,16 L26,30 L10,58 Q9,62 14,62 L50,62 Q55,62 54,58 L38,30 L38,16 Z" />
        </clipPath>
      </defs>

      {/* Steam wisps above the neck — hidden while dormant */}
      <ellipse style={steamStyle('0s')} cx="24" cy="9" rx="3" ry="6" fill="rgba(190,242,100,0.4)" filter="url(#boosterSteamBlur)" />
      <ellipse style={steamStyle('0.7s')} cx="31" cy="4" rx="2.6" ry="5.5" fill="rgba(226,255,230,0.4)" filter="url(#boosterSteamBlur)" />
      <ellipse style={steamStyle('1.4s')} cx="37" cy="9" rx="2.6" ry="5.5" fill="rgba(190,242,100,0.35)" filter="url(#boosterSteamBlur)" />

      {/* Liquid, clipped to the flask body — grey and still while dormant */}
      <g clipPath="url(#boosterFlaskClip)">
        <rect x="6" y="32" width="52" height="32" fill={`url(#${dormant ? 'boosterLiquidGrey' : 'boosterLiquid'})`} />
        <circle style={bubbleStyle('0s')} cx="20" cy="55" r="2" fill="#bef264" />
        <circle style={bubbleStyle('0.4s')} cx="30" cy="52" r="1.5" fill="#dcfce7" />
        <circle style={bubbleStyle('0.8s')} cx="41" cy="57" r="2.2" fill="#bef264" />
        <circle style={bubbleStyle('1.2s')} cx="26" cy="47" r="1.3" fill="#dcfce7" />
        <circle style={bubbleStyle('1.6s')} cx="46" cy="45" r="1.6" fill="#bef264" />
      </g>

      {/* Glass outline drawn on top so the liquid stays inside it */}
      <path
        d="M26,16 L26,30 L10,58 Q9,62 14,62 L50,62 Q55,62 54,58 L38,30 L38,16 Z"
        fill="none"
        stroke={dormant ? 'rgba(148,163,184,0.4)' : 'rgba(191,219,254,0.55)'}
        strokeWidth="2"
        strokeLinejoin="round"
      />
      <rect x="24" y="14" width="16" height="4" rx="1.5" fill={dormant ? 'rgba(148,163,184,0.3)' : 'rgba(191,219,254,0.35)'} />

      {/* Keyframes only — safe to declare identically across multiple
          mounted instances (widget + modal can both be on screen at once).
          Whether they actually apply is controlled per-element above via
          inline `animation`, not by toggling a shared class name, so the
          two instances can never fight over conflicting states. */}
      <style>{`
        @keyframes boosterBubbleRise {
          0%   { transform: translateY(0) scale(0.8); opacity: 0; }
          15%  { opacity: 1; }
          100% { transform: translateY(-26px) scale(1); opacity: 0; }
        }
        @keyframes boosterSteamRise {
          0%   { transform: translateY(0) scale(0.8); opacity: 0.55; }
          100% { transform: translateY(-15px) scale(1.35); opacity: 0; }
        }
      `}</style>
    </svg>
  );
...
const formatBoosterCooldown = (totalSeconds) => {
...
const BoosterPackageCTA = ({ playerAddress }) => {
  const [dismissed, setDismissed] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const [cooldown, setCooldown] = useState(null); // null = not loaded yet
  ...
  const fetchStatus = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/booster/status/${playerAddress}`);
      if (res.ok) {
        const data = await res.json();
        setCooldown(data);
    ...
    } catch (e) { /* non-fatal — widget just shows as available */ }
  }, [playerAddress]);

  useEffect(() => {
    fetchStatus();
    const poll = setInterval(fetchStatus, 5 * 60 * 1000); // resync every 5 min
    return () => clearInterval(poll);
  }, [fetchStatus]);

  // Live countdown between polls — 30s granularity is plenty for a 48h cap.
  useEffect(() => {
    if (!cooldown?.on_cooldown) return undefined;
    const tick = setInterval(() => {
      const elapsed = Math.floor((Date.now() - fetchedAtRef.current) / 1000);
      const remaining = Math.max(0, (cooldown.seconds_remaining || 0) - elapsed);
      setSecondsLeft(remaining);
      if (remaining <= 0) {
        setCooldown((prev) => (prev ? { ...prev, on_cooldown: false } : prev));
      }
    }, 30000);
  ...
  const onCooldown = !!cooldown?.on_cooldown;

  return (
    <>
      {/* Sits just above the "?" Game Guide button (bottom:90, left:16) */}
      <div style={{ position: 'fixed', left: 14, bottom: 138, zIndex: 998 }}>
        <div
          style={{
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
            filter: onCooldown ? 'none' : 'drop-shadow(0 4px 14px rgba(74,222,128,0.35))',
            animation: onCooldown ? 'none' : 'boosterBob 3s ease-in-out infinite',
          }}
        >
          <style>{`
          ...
          {/* The booster bubble — greys out and shows a countdown on cooldown */}
          <button
            ...
            style={{
              ...
              background: onCooldown
                ...
                : 'linear-gradient(160deg, #151b28 0%, #0d1117 100%)',
              border: onCooldown ? '2px solid rgba(148,163,184,0.35)' : '2px solid rgba(74,222,128,0.45)',
              borderRadius: 20,
              padding: '9px 10px 8px',
              cursor: 'pointer',
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
              minWidth: 78,
              boxShadow: onCooldown ? 'none' : '0 0 16px rgba(74,222,128,0.22), inset 0 1px 0 rgba(255,255,255,0.04)',
            }}
            data-testid="booster-widget-open"
            aria-label="Booster Package"
          >
            <BoosterFlaskIcon size={40} dormant={onCooldown} />
            <span style={{
              fontSize: 9, fontWeight: 800, color: onCooldown ? 'rgba(255,255,255,0.45)' : '#bef264',
            ...
            <span style={{
              fontSize: 9, fontWeight: 700, color: onCooldown ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.55)',
              lineHeight: 1, whiteSpace: 'nowrap',
            }}>
              {onCooldown ? formatBoosterCooldown(secondsLeft) : `${BOOSTER_PACKAGE_UI.costDoge} DOGE`}
            </span>
          </button>
        </div>
      </div>
...
const BoosterPurchaseModal = ({ playerAddress, onClose, onCooldown = false, secondsLeft = 0 }) => {
  ...
  const handleBuy = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${BACKEND_URL}/api/nowpayments/booster/create`, {
        method: 'POST',
    ...
    >
      ...
      >
        <div
          ...
          style={{ background: 'linear-gradient(160deg, #151b28 0%, #0d1117 100%)', borderBottom: '1px solid rgba(74,222,128,0.25)' }}
        >
          <div
            className="absolute rounded-full pointer-events-none"
            style={{ width: 160, height: 160, top: -60, left: '50%', transform: 'translateX(-50%)', background: onCooldown ? 'rgba(148,163,184,0.12)' : 'rgba(74,222,128,0.18)', filter: 'blur(30px)' }}
          />
          <button
            onClick={onClose}
          ...
          <div className="relative flex items-center justify-center mb-1">
            <BoosterFlaskIcon size={56} dormant={onCooldown} />
          </div>
          <h3 className="relative text-lg font-bold text-white">Booster Pack</h3>
          <p className="relative text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.55)' }}>
            {onCooldown ? `On cooldown — ${formatBoosterCooldown(secondsLeft)}` : 'Everything you need to get ahead, all at once'}
          </p>
        </div>

        <div className="p-5 space-y-3">
          <div className="flex items-center gap-3 p-2.5 rounded-xl" style={{ backgroundColor: 'rgba(255,255,255,0.04)' }}>
            <Zap className="w-5 h-5 text-sky-400 shrink-0" />
            <div>
              <div className="text-sm font-bold text-white">{BOOSTER_PACKAGE_UI.autoMixerDays}-Day Auto-Mixer</div>
              <div className="text-[11px] text-slate-400">Mixes treats for you automatically</div>
            </div>
          </div>
          <div className="flex items-center gap-3 p-2.5 rounded-xl" style={{ backgroundColor: 'rgba(255,255,255,0.04)' }}>
            <Beaker className="w-5 h-5 text-emerald-400 shrink-0" />
            <div>
              <div className="text-sm font-bold text-white">{BOOSTER_PACKAGE_UI.extraLives} Extra Lives</div>
              <div className="text-[11px] text-slate-400">More treat creations beyond your daily limit</div>
            </div>
          </div>
          <div className="flex items-center gap-3 p-2.5 rounded-xl" style={{ backgroundColor: 'rgba(255,255,255,0.04)' }}>
            <Gem className="w-5 h-5 text-amber-400 shrink-0" />
            <div>
              <div className="text-sm font-bold text-white">{BOOSTER_PACKAGE_UI.points.toLocaleString()} Points</div>
              <div className="text-[11px] text-slate-400">Credited straight to your balance</div>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3 rounded-2xl font-bold text-sm disabled:opacity-60 transition-transform active:scale-[0.98]"
            style={{
              background: onCooldown
                ? 'linear-gradient(160deg, #334155 0%, #1e293b 100%)'
                : 'linear-gradient(160deg, #86efac 0%, #22c55e 55%, #15803d 100%)',
              color: onCooldown ? 'rgba(255,255,255,0.6)' : '#052e16',
              border: onCooldown ? '2px solid rgba(148,163,184,0.35)' : '2px solid #0b1738',
              boxShadow: onCooldown ? 'none' : '0 5px 0 #14532d',
              cursor: onCooldown ? 'not-allowed' : 'pointer',
            }}
          >
            {onCooldown
              ? `Available in ${formatBoosterCooldown(secondsLeft)}`
              : busy ? 'Starting checkout…' : `Buy for ${BOOSTER_PACKAGE_UI.costDoge} DOGE`}
          </button>
          <p className="text-[10px] text-center text-slate-500">
            {onCooldown
              ? `One Booster Pack per player every ${BOOSTER_COOLDOWN_HOURS_UI} hours.`
              : 'Paid via NOWPayments — same secure checkout used across DogeFood Lab.'}
          </p>
          {!onCooldown && BOOSTER_PACKAGE_UI.autoMixerDays > 0 && (
            <p className="text-[10px] text-center text-slate-500">Already have an active Auto-Mixer? The {BOOSTER_PACKAGE_UI.autoMixerDays} days stack on top of your remaining time.</p>
          )}
        </div>
      </div>
    </div>
...
const MainMenu = ({ playerAddress: playerAddressProp } = {}) => {
  ...
  const [heatEvent, setHeatEvent] = useState(null);
  ...
  // Arena heat event rotates every 30 min — poll for it separately so the
  // Arena Event card on the main menu stays current without a full reload.
  useEffect(() => {
    const fetchHeat = () => {
      fetch(`${BACKEND_URL}/api/arena/heat`)
        .then(r => r.ok ? r.json() : null)
        .then(h => { if (h) setHeatEvent(h); })
        .catch(() => {});
    };
    fetchHeat();
    const poll = setInterval(fetchHeat, 60000);
    return () => clearInterval(poll);
  }, []);
    ...
    <div className="min-h-screen bg-[#0d1117]" data-testid="main-menu">
      ...
      <div className="max-w-[1600px] mx-auto flex">
        ...
        <main className="flex-1 min-w-0 px-3 sm:px-5 py-4 space-y-4 sm:space-y-5">
          ...
          {/* ── Banner Carousel ── */}
          <BannerCarousel banners={MENU_BANNERS} onAuthRequired={handleLabAccess} />
          ...
          {/* ── Featured Banner (Lab CTA + Happy Hour + Arena Event) ── */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      ...
      <SpinWheelCTA />
      <BoosterPackageCTA playerAddress={effectiveAddress} />
