...
const BoosterFlaskIcon = ({ size = 44 }) => (
  <svg viewBox="0 0 64 72" width={size} height={size * (72 / 64)} style={{ overflow: 'visible' }}>
    <defs>
      <linearGradient id="boosterLiquid" x1="0" y1="1" x2="0" y2="0">
    ...
    {/* Steam wisps above the neck */}
    <ellipse className="booster-steam booster-steam-1" cx="24" cy="9" rx="3" ry="6" fill="rgba(190,242,100,0.4)" filter="url(#boosterSteamBlur)" />
    <ellipse className="booster-steam booster-steam-2" cx="31" cy="4" rx="2.6" ry="5.5" fill="rgba(226,255,230,0.4)" filter="url(#boosterSteamBlur)" />
    <ellipse className="booster-steam booster-steam-3" cx="37" cy="9" rx="2.6" ry="5.5" fill="rgba(190,242,100,0.35)" filter="url(#boosterSteamBlur)" />

    {/* Bubbling liquid, clipped to the flask body */}
    <g clipPath="url(#boosterFlaskClip)">
      <rect x="6" y="32" width="52" height="32" fill="url(#boosterLiquid)" />
      <circle className="booster-bubble booster-bubble-1" cx="20" cy="55" r="2" fill="#bef264" />
      <circle className="booster-bubble booster-bubble-2" cx="30" cy="52" r="1.5" fill="#dcfce7" />
      <circle className="booster-bubble booster-bubble-3" cx="41" cy="57" r="2.2" fill="#bef264" />
      <circle className="booster-bubble booster-bubble-4" cx="26" cy="47" r="1.3" fill="#dcfce7" />
      <circle className="booster-bubble booster-bubble-5" cx="46" cy="45" r="1.6" fill="#bef264" />
    </g>
    ...
    />
    <rect x="24" y="14" width="16" height="4" rx="1.5" fill="rgba(191,219,254,0.35)" />

    <style>{`
      .booster-bubble, .booster-steam { transform-box: fill-box; transform-origin: center; }
      @keyframes boosterBubbleRise {
        0%   { transform: translateY(0) scale(0.8); opacity: 0; }
        15%  { opacity: 1; }
        100% { transform: translateY(-26px) scale(1); opacity: 0; }
      }
      .booster-bubble-1 { animation: boosterBubbleRise 2.1s ease-in infinite; animation-delay: 0s; }
      .booster-bubble-2 { animation: boosterBubbleRise 2.1s ease-in infinite; animation-delay: 0.4s; }
      .booster-bubble-3 { animation: boosterBubbleRise 2.1s ease-in infinite; animation-delay: 0.8s; }
      .booster-bubble-4 { animation: boosterBubbleRise 2.1s ease-in infinite; animation-delay: 1.2s; }
      .booster-bubble-5 { animation: boosterBubbleRise 2.1s ease-in infinite; animation-delay: 1.6s; }
      ...
      .booster-steam-1 { animation: boosterSteamRise 2.6s ease-out infinite; animation-delay: 0s; }
      .booster-steam-2 { animation: boosterSteamRise 2.6s ease-out infinite; animation-delay: 0.7s; }
      .booster-steam-3 { animation: boosterSteamRise 2.6s ease-out infinite; animation-delay: 1.4s; }
    `}</style>
  </svg>
);
    ...
    <>
      ...
      <div style={{ position: 'fixed', left: 14, bottom: 138, zIndex: 998 }}>
        <div
          style={{
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
            filter: 'drop-shadow(0 4px 14px rgba(74,222,128,0.35))',
            animation: 'boosterBob 3s ease-in-out infinite',
          }}
        >
          <style>{`
          ...
          <button
            ...
            style={{
              position: 'relative',
              background: 'linear-gradient(160deg, #151b28 0%, #0d1117 100%)',
              ...
              minWidth: 78,
              boxShadow: '0 0 16px rgba(74,222,128,0.22), inset 0 1px 0 rgba(255,255,255,0.04)',
            }}
            data-testid="booster-widget-open"
            aria-label="Booster Package"
          >
            <BoosterFlaskIcon size={40} />
            <span style={{
              fontSize: 9, fontWeight: 800, color: '#bef264',
            ...
            <span style={{
              ...
              {BOOSTER_PACKAGE_UI.costDoge} DOGE
            </span>
          </button>
        </div>
      </div>
      ...
      {showModal && (
        <BoosterPurchaseModal playerAddress={playerAddress} onClose={() => setShowModal(false)} />
      )}
    </>
  );
};

const BoosterPurchaseModal = ({ playerAddress, onClose }) => {
      ...
      >
        ...
        >
          <div
            ...
            style={{ width: 160, height: 160, top: -60, left: '50%', transform: 'translateX(-50%)', background: 'rgba(74,222,128,0.18)', filter: 'blur(30px)' }}
          />
          <button
            onClick={onClose}
          ...
          <div className="relative flex items-center justify-center mb-1">
            <BoosterFlaskIcon size={56} />
          </div>
          <h3 className="relative text-lg font-bold text-white">Booster Pack</h3>
          <p className="relative text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.55)' }}>Everything you need to get ahead, all at once</p>
        </div>
        ...
        <div className="p-5 space-y-3">
          ...
          <button
            ...
            style={{
              background: 'linear-gradient(160deg, #86efac 0%, #22c55e 55%, #15803d 100%)',
              color: '#052e16',
              border: '2px solid #0b1738',
              boxShadow: '0 5px 0 #14532d',
            }}
          >
            {busy ? 'Starting checkout…' : `Buy for ${BOOSTER_PACKAGE_UI.costDoge} DOGE`}
          </button>
          <p className="text-[10px] text-center text-slate-500">Paid via NOWPayments — same secure checkout used across DogeFood Lab.</p>
          {BOOSTER_PACKAGE_UI.autoMixerDays > 0 && (
